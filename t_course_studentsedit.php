...
$t_course_students_edit = NULL; // Initialize page object first

class ct_course_students_edit extends ct_course_students {
	...
	// Page object name
	var $PageObjName = 't_course_students_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			$this->Page_Terminate("t_course_studentslist.php");
		}
	...
	function Page_Main() {
		...
		if (@$_GET["CourseStudentID"] <> "") {
			...
			$this->RecKey["CourseStudentID"] = $this->CourseStudentID->QueryStringValue;
		} else {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("t_course_studentslist.php"); // Return to list page
			...
			while (!$this->Recordset->EOF) {
				if (strval($this->CourseStudentID->CurrentValue) == strval($this->Recordset->fields('CourseStudentID'))) {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "t_course_studentsview.php")
	...
	function LoadFormValues() {
		...
		if (!$this->CourseStudentID->FldIsDetailKey)
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "t_course_studentslist.php", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($t_course_students_edit)) $t_course_students_edit = new ct_course_students_edit();

// Page init
$t_course_students_edit->Page_Init();

// Page main
$t_course_students_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_course_students_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var t_course_students_edit = new ew_Page("t_course_students_edit");
t_course_students_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = t_course_students_edit.PageID; // For backward compatibility

// Form object
var ft_course_studentsedit = new ew_Form("ft_course_studentsedit");

// Validate form
ft_course_studentsedit.Validate = function() {
...
ft_course_studentsedit.Form_CustomValidate = 
...
ft_course_studentsedit.ValidateRequired = true;
<?php } else { ?>
ft_course_studentsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_course_studentsedit.Lists["x_CourseID"] = {"LinkField":"x_CourseID","Ajax":true,"AutoFill":false,"DisplayFields":["x_CourseCode","x_Course","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ft_course_studentsedit.Lists["x_StudentID"] = {"LinkField":"x__UserID","Ajax":true,"AutoFill":false,"DisplayFields":["x_RegistrationNumber","x_LastName","x_FirstName",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $t_course_students_edit->ShowPageHeader(); ?>
<?php
$t_course_students_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($t_course_students_edit->Pager)) $t_course_students_edit->Pager = new cPrevNextPager($t_course_students_edit->StartRec, $t_course_students_edit->DisplayRecs, $t_course_students_edit->TotalRecs) ?>
<?php if ($t_course_students_edit->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
	<?php echo $Language->Phrase("Page") ?>&nbsp;
<div class="input-prepend input-append">
<!--first page button-->
	<?php if ($t_course_students_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $t_course_students_edit->PageUrl() ?>start=<?php echo $t_course_students_edit->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_course_students_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $t_course_students_edit->PageUrl() ?>start=<?php echo $t_course_students_edit->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_course_students_edit->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($t_course_students_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $t_course_students_edit->PageUrl() ?>start=<?php echo $t_course_students_edit->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_course_students_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $t_course_students_edit->PageUrl() ?>start=<?php echo $t_course_students_edit->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_course_students_edit->Pager->PageCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
...
<form name="ft_course_studentsedit" id="ft_course_studentsedit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="t_course_students">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_t_course_studentsedit" class="table table-bordered table-striped">
<?php if ($t_course_students->CourseID->Visible) { // CourseID ?>
	<tr id="r_CourseID">
		<td><span id="elh_t_course_students_CourseID"><?php echo $t_course_students->CourseID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $t_course_students->CourseID->CellAttributes() ?>>
<span id="el_t_course_students_CourseID" class="control-group">
<select data-field="x_CourseID" id="x_CourseID" name="x_CourseID"<?php echo $t_course_students->CourseID->EditAttributes() ?>>
<?php
...
&nbsp;<a id="aol_x_CourseID" class="ewAddOptLink" href="javascript:void(0);" onclick="ew_AddOptDialogShow({lnk:this,el:'x_CourseID',url:'t_coursesaddopt.php'});"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $t_course_students->CourseID->FldCaption() ?></a>
<?php } ?>
<?php
$sSqlWrk = "SELECT `CourseID`, `CourseCode` AS `DispFld`, `Course` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t_courses`";
$sWhereWrk = "";
...
<?php echo $t_course_students->CourseID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_course_students->StudentID->Visible) { // StudentID ?>
	<tr id="r_StudentID">
		<td><span id="elh_t_course_students_StudentID"><?php echo $t_course_students->StudentID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $t_course_students->StudentID->CellAttributes() ?>>
<span id="el_t_course_students_StudentID" class="control-group">
<?php
...
?>
<span id="as_x_StudentID" style="white-space: nowrap; z-index: 8970">
	<input type="text" name="sv_x_StudentID" id="sv_x_StudentID" value="<?php echo $t_course_students->StudentID->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($t_course_students->StudentID->PlaceHolder) ?>"<?php echo $t_course_students->StudentID->EditAttributes() ?>>&nbsp;<span id="em_x_StudentID" class="ewMessage" style="display: none"><?php echo str_replace("%f", "phpimages/", $Language->Phrase("UnmatchedValue")) ?></span>
	<div id="sc_x_StudentID" style="display: inline; z-index: 8970"></div>
</span>
<input type="hidden" data-field="x_StudentID" name="x_StudentID" id="x_StudentID" value="<?php echo ew_HtmlEncode($t_course_students->StudentID->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<?php
...
?>
<input type="hidden" name="q_x_StudentID" id="q_x_StudentID" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>">
<script type="text/javascript">
var oas = new ew_AutoSuggest("x_StudentID", ft_course_studentsedit, true, EW_AUTO_SUGGEST_MAX_ENTRIES);
oas.formatResult = function(ar) {
	var dv = ar[1];
	for (var i = 2; i <= 4; i++)
		dv += (ar[i]) ? ew_ValueSeparator(i - 1, "x_StudentID") + ar[i] : "";
	return dv;
}
ft_course_studentsedit.AutoSuggests["x_StudentID"] = oas;
</script>
<?php if (AllowAdd(CurrentProjectID() . "t_users")) { ?>
&nbsp;<a id="aol_x_StudentID" class="ewAddOptLink" href="javascript:void(0);" onclick="ew_AddOptDialogShow({lnk:this,el:'x_StudentID',url:'t_usersaddopt.php'});"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $t_course_students->StudentID->FldCaption() ?></a>
<?php } ?>
<?php
$sSqlWrk = "SELECT `UserID`, `RegistrationNumber` AS `DispFld`, `LastName` AS `Disp2Fld`, `FirstName` AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t_users`";
$sWhereWrk = "{filter}";
...
$sSqlWrk .= " ORDER BY `RegistrationNumber`";
?>
<input type="hidden" name="s_x_StudentID" id="s_x_StudentID" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&amp;f0=<?php echo ew_Encrypt("`UserID` = {filter_value}"); ?>&amp;t0=3">
</span>
<?php echo $t_course_students->StudentID->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<input type="hidden" data-field="x_CourseStudentID" name="x_CourseStudentID" id="x_CourseStudentID" value="<?php echo ew_HtmlEncode($t_course_students->CourseStudentID->CurrentValue) ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($t_course_students_edit->Pager)) $t_course_students_edit->Pager = new cPrevNextPager($t_course_students_edit->StartRec, $t_course_students_edit->DisplayRecs, $t_course_students_edit->TotalRecs) ?>
<?php if ($t_course_students_edit->Pager->RecordCount > 0) { ?>
...
<script type="text/javascript">
ft_course_studentsedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$t_course_students_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_course_students_edit->Page_Terminate();
?>
