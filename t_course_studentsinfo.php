...
$t_course_students = NULL;

//
// Table class for t_course_students
//
class ct_course_students extends cTable {
	var $CourseStudentID;
	var $CourseID;
	var $StudentID;
	...
	function __construct() {
		...
		$this->TableVar = 't_course_students';
		$this->TableName = 't_course_students';
		...
		// CourseStudentID
		$this->CourseStudentID = new cField('t_course_students', 't_course_students', 'x_CourseStudentID', 'CourseStudentID', '`CourseStudentID`', '`CourseStudentID`', 3, -1, FALSE, '`CourseStudentID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->CourseStudentID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['CourseStudentID'] = &$this->CourseStudentID;

		// CourseID
		$this->CourseID = new cField('t_course_students', 't_course_students', 'x_CourseID', 'CourseID', '`CourseID`', '`CourseID`', 3, -1, FALSE, '`EV__CourseID`', TRUE, TRUE, FALSE, 'FORMATTED TEXT');
		$this->CourseID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['CourseID'] = &$this->CourseID;

		// StudentID
		$this->StudentID = new cField('t_course_students', 't_course_students', 'x_StudentID', 'StudentID', '`StudentID`', '`StudentID`', 3, -1, FALSE, '`StudentID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlSelectList() { // Select for List page
		return "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`CourseCode`,'" . ew_ValueSeparator(1, $this->CourseID) . "',`Course`) FROM `t_courses` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`CourseID` = `t_course_students`.`CourseID` LIMIT 1) AS `EV__CourseID` FROM `t_course_students`" .
			") `EW_TMP_TABLE`";
	}
	...
	// Update Table
	var $UpdateTable = "`t_course_students`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('CourseStudentID', $rs))
				ew_AddFilter($where, ew_QuotedName('CourseStudentID') . '=' . ew_QuotedValue($rs['CourseStudentID'], $this->CourseStudentID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`CourseStudentID` = @CourseStudentID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->CourseStudentID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@CourseStudentID@", ew_AdjustSql($this->CourseStudentID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t_course_studentslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("t_course_studentsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("t_course_studentsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "t_course_studentsadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("t_course_studentsedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("t_course_studentsadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_course_studentsdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->CourseStudentID->CurrentValue)) {
			$sUrl .= "CourseStudentID=" . urlencode($this->CourseStudentID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["CourseStudentID"]; // CourseStudentID
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->CourseStudentID->CurrentValue = $key;
   ...
   // Common render codes
		...
		$this->CourseStudentID->ViewValue = $this->CourseStudentID->CurrentValue;
		$this->CourseStudentID->ViewCustomAttributes = "";
		...
		// CourseStudentID
		$this->CourseStudentID->LinkCustomAttributes = "";
		$this->CourseStudentID->HrefValue = "";
		$this->CourseStudentID->TooltipValue = "";
